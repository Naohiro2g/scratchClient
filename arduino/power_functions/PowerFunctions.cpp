...
void PowerFunctions::single_pwm(uint8_t output, uint8_t pwm) {
  _nib1 = _toggle | _escape | _channel;
...
void PowerFunctions::combo_pwm(uint8_t blue_speed, uint8_t red_speed)
{
  ...
  send();
   ...
   void PowerFunctions::single_cmd(uint8_t mode, uint8_t data)
   {
     _nib1 = _escape | _channel;
  _nib2 = _address | mode;
  _nib3 = data;
  send();
   }

   void PowerFunctions::sendData(uint16_t message)
   {
    if ( message == 0 ) return;
